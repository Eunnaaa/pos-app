...
import { StockOpnameTab } from "./stock-opname-tab"
...
export function InventoryPage() {
  ...
  return <div className="flex flex-1 flex-col gap-5 p-4 md:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-bold">Inventory & Stok</h2><p className="text-sm text-muted-foreground">Saldo, mutasi, dan stock opname untuk {warehouse?.name || "gudang aktif"}.</p></div>{isOwner && <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setOpen(true)}><SlidersHorizontal /> Penyesuaian stok manual</Button>}</div>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Card className="py-4 shadow-sm"><CardContent className="px-5 py-0"><p className="text-sm text-muted-foreground">SKU tercatat</p><p className="mt-1 text-2xl font-bold">{visibleBalances.length}</p></CardContent></Card>
      <Card className="py-4 shadow-sm"><CardContent className="px-5 py-0"><p className="text-sm text-muted-foreground">Total unit tersedia</p><p className="mt-1 text-2xl font-bold">{visibleBalances.reduce((sum, item) => sum + Number(item.available), 0)}</p></CardContent></Card>
      <Card className="py-4 shadow-sm"><CardContent className="px-5 py-0"><p className="text-sm text-muted-foreground">Stok menipis</p><p className="mt-1 text-2xl font-bold text-amber-600">{low.length}</p></CardContent></Card>
      <Card className="py-4 shadow-sm"><CardContent className="px-5 py-0"><p className="text-sm text-muted-foreground">Nilai stok (HPP)</p><p className="mt-1 text-2xl font-bold">Rp {Number(inventoryValue).toLocaleString("id-ID")}</p></CardContent></Card>
    </div>
    <Card className="shadow-sm"><Tabs defaultValue="stock"><CardHeader className="border-b pb-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><TabsList><TabsTrigger value="stock">Stok Saldo</TabsTrigger><TabsTrigger value="movement">Mutasi Stok</TabsTrigger><TabsTrigger value="opname">Stock Opname (SO)</TabsTrigger></TabsList><div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari SKU/produk" className="pl-9 sm:w-72" /></div></div></CardHeader><CardContent className="p-0">
      <TabsContent value="stock" className="m-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Produk / SKU</TableHead><TableHead>On hand</TableHead><TableHead>Reserved</TableHead><TableHead>Available</TableHead><TableHead>Reorder point</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{loading && <TableRow><TableCell colSpan={6} className="h-32 text-center"><Loader2 className="mx-auto animate-spin" /></TableCell></TableRow>}{!loading && !visibleBalances.length && <TableRow><TableCell colSpan={6} className="h-32 text-center text-muted-foreground">Belum ada saldo stok. Tambahkan stok awal dari halaman produk atau adjustment.</TableCell></TableRow>}{visibleBalances.map((item) => { const isLow = BigInt(item.available) <= BigInt(item.reorder_point); return <TableRow key={item.id}><TableCell className="font-medium">{names.get(item.variant_id) || item.variant_id}</TableCell><TableCell>{item.on_hand}</TableCell><TableCell>{item.reserved}</TableCell><TableCell>{item.available}</TableCell><TableCell>{item.reorder_point}</TableCell><TableCell><Badge variant={isLow ? "secondary" : "default"} className={isLow ? "text-amber-700" : "bg-emerald-600"}>{isLow ? "Menipis" : "Aman"}</Badge></TableCell></TableRow> })}</TableBody></Table></div></TabsContent>
      <TabsContent value="movement" className="m-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Waktu</TableHead><TableHead>Produk / SKU</TableHead><TableHead>Tipe</TableHead><TableHead>Qty</TableHead><TableHead>Sebelum</TableHead><TableHead>Sesudah</TableHead><TableHead>Alasan</TableHead></TableRow></TableHeader><TableBody>{movements.data.filter((item) => names.has(item.variant_id)).map((item) => <TableRow key={item.id}><TableCell>{new Date(item.occurred_at).toLocaleString("id-ID")}</TableCell><TableCell className="font-medium">{names.get(item.variant_id)}</TableCell><TableCell><Badge variant="outline">{item.type}</Badge></TableCell><TableCell className={BigInt(item.quantity) >= 0n ? "text-emerald-600" : "text-rose-600"}>{BigInt(item.quantity) >= 0n ? <ArrowUp className="mr-1 inline size-3" /> : <ArrowDown className="mr-1 inline size-3" />}{item.quantity}</TableCell><TableCell>{item.before_quantity}</TableCell><TableCell>{item.after_quantity}</TableCell><TableCell>{item.reason || "—"}</TableCell></TableRow>)}</TableBody></Table></div></TabsContent>
      <TabsContent value="opname" className="p-5"><StockOpnameTab /></TabsContent>
    </CardContent></Tabs></Card>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><form onSubmit={adjust}><DialogHeader><DialogTitle className="flex items-center gap-2"><Boxes className="text-emerald-600" /> Penyesuaian stok manual</DialogTitle><DialogDescription>Gunakan angka positif untuk menambah, negatif untuk mengurangi.</DialogDescription></DialogHeader><div className="space-y-4 py-5"><div className="space-y-2"><Label>Produk / varian</Label><Select value={form.variantId} onValueChange={(value) => setForm((current) => ({ ...current, variantId: value }))}><SelectTrigger><SelectValue placeholder="Pilih produk" /></SelectTrigger><SelectContent>{variants.data.map((item) => <SelectItem key={item.id} value={item.id}>{names.get(item.id)}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Perubahan kuantitas</Label><Input type="number" value={form.quantity} onChange={(event) => setForm((current) => ({ ...current, quantity: event.target.value }))} placeholder="Contoh: 10 atau -2" required /></div><div className="space-y-2"><Label>Alasan</Label><Input value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} placeholder="Stok awal / barang rusak / koreksi" required minLength={3} /></div></div><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Batal</Button><Button className="bg-emerald-600 hover:bg-emerald-700" disabled={saving || !form.variantId}>{saving && <Loader2 className="animate-spin" />} Simpan</Button></DialogFooter></form></DialogContent></Dialog>
  </div>
}
