...
export function ProductsPage() {
  ...
  return <div className="flex flex-1 flex-col gap-5 p-4 md:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-bold">Produk</h2><p className="text-sm text-muted-foreground">Data produk dan varian langsung dari database.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={exportCSV}><Download className="size-4" /> Export CSV</Button><Button variant="outline" size="sm" onClick={() => setImportOpen(true)}><Upload className="size-4" /> Import CSV</Button><Button className="bg-emerald-600 hover:bg-emerald-700" onClick={showCreate}><Plus /> Tambah produk</Button></div></div>
    ...
    <Dialog open={importOpen} onOpenChange={setImportOpen}><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2"><FileSpreadsheet className="text-emerald-600" /> Import Produk dari CSV</DialogTitle><DialogDescription>Tempelkan data CSV. Format: name,sku,barcode,price,cost,description,track_stock,is_active</DialogDescription></DialogHeader><div className="space-y-3 py-3"><div className="flex justify-end"><Button type="button" variant="outline" size="sm" onClick={downloadTemplate}><Download className="size-4" /> Download Template</Button></div><textarea className="min-h-[200px] w-full rounded-lg border p-3 font-mono text-xs" placeholder={"name,sku,barcode,price,cost,description,track_stock,is_active\nKopi Susu,SKU-001,8990001234567,25000,15000,,true,true\nCroissant,SKU-002,,20000,10000,Pastry,true,true"} value={csvText} onChange={(event) => setCsvText(event.target.value)} /></div><DialogFooter><Button type="button" variant="outline" onClick={() => setImportOpen(false)}>Batal</Button><Button type="button" className="bg-emerald-600 hover:bg-emerald-700" disabled={importing || !csvText.trim()} onClick={() => void handleImport()}>{importing ? <><Loader2 className="size-4 animate-spin" /> Mengimpor...</> : <><Upload className="size-4" /> Import</>}</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
