...
type CashSession = { id: string; openingAmount: string; openedAt: string; registerName: string; registerCode: string }
...
export function PosScreen() {
  ...
  const loadSession = useCallback(async () => {
    ...
    setSessionError("")
    try {
      const [activeResponse, registerResponse] = await Promise.all([
        apiFetch<CashSession | null>("/api/v1/finance/cash-sessions/active"),
        apiFetch<CashRegister[]>("/api/v1/finance/registers"),
      ])
      
      // Jika sudah ada session aktif, tampilkan
      if (activeResponse.data) {
        setSession(activeResponse.data)
        return
      }

      // Jika belum ada session, coba auto-open dengan register pertama
      if (!autoOpenAttempted && registerResponse.data.length > 0) {
        setAutoOpenAttempted(true)
        const defaultRegister = registerResponse.data[0]
        try {
          const openResponse = await apiFetch<CashSession>("/api/v1/finance/cash-sessions", {
            method: "POST",
            body: JSON.stringify({ registerId: defaultRegister.id, openingAmount: "0" }),
          })
          setSession(openResponse.data)
          toast.success("Shift kasir dibuka otomatis")
        } catch (autoOpenError) {
          const message = autoOpenError instanceof Error ? autoOpenError.message : "Gagal auto-open shift"
          setSessionError(message)
          toast.error(message)
        }
      } else if (registerResponse.data.length === 0) {
        setSessionError("Mesin kasir tidak tersedia. Buat cash register untuk cabang ini terlebih dahulu.")
      }
    } catch (caught) {
  ...
  async function recordMovement(event: React.FormEvent) {
    ...
    try {
      ...
      toast.success(movement.direction === "in" ? "Kas masuk dicatat" : "Kas keluar dicatat")
      setMovement({ direction: "in", amount: "", category: "", reason: "" })
      setShiftOpen(false)
    } catch (caught) {
      toast.error(caught instanceof Error ? caught.message : "Gagal mencatat mutasi kas")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function closeShift(event: React.FormEvent) {
    event.preventDefault()
    if (!session) return
    if (cart.length) return toast.error("Selesaikan atau kosongkan keranjang sebelum menutup shift")
    const actuals = Object.fromEntries(Object.entries(tenderActuals).filter(([, value]) => value !== ""))
    if (actuals.cash === undefined) return toast.error("Kas aktual wajib diisi")
    setSubmitting(true)
    try {
      ...
      toast.error(caught instanceof Error ? caught.message : "Gagal menutup shift")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  function add(product: Product) {
    if (product.trackStock && product.stock <= 0) return toast.error("Stok produk habis")
    setCart((current) => {
      const exists = current.find((item) => item.id === product.id)
      if (exists) {
        if (product.trackStock && exists.quantity >= product.stock) { toast.error("Jumlah melebihi stok tersedia"); return current }
  ...
  function change(id: string, changeBy: number) {
    setCart((current) => current.map((item) => {
      ...
      if (item.trackStock && next > item.stock) { toast.error("Jumlah melebihi stok tersedia"); return item }
      return { ...item, quantity: next }
    }).filter((item) => item.quantity > 0))
  }

  async function submitOrder(status: "paid" | "held") {
    if (!branch?.id || !warehouse?.id) return toast.error("Cabang atau gudang belum dipilih")
    if (!session) return toast.error("Buka shift kasir sebelum transaksi")
    if (!cart.length) return
    if (status === "paid" && paymentMethod === "Tunai" && cash < total) return toast.error("Nominal tunai belum cukup")
    setSubmitting(true)
    try {
      ...
      if (response.queued) {
        toast.warning("Transaksi disimpan offline", { description: "Akan disinkronkan saat koneksi kembali." })
        setPaymentOpen(false); setCart([]); setOrderNote(""); setDiscount("0"); return
      }
      toast.success(status === "held" ? "Pesanan ditahan" : "Pembayaran berhasil")
    ...
    } catch (caught) { toast.error(caught instanceof Error ? caught.message : "Transaksi gagal") }
  ...
  if (!session) {
    return <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-muted/30 p-4"><Card className="w-full max-w-lg shadow-xl"><CardContent className="p-7"><div className="text-center"><span className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700"><Store className="size-8" /></span><h2 className="mt-5 text-2xl font-bold">Mempersiapkan shift kasir</h2><p className="mt-1 text-sm text-muted-foreground">Mohon tunggu, shift kasir sedang dibuka...</p></div>{sessionError && <div className="mt-5 rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive"><p className="font-medium">Gagal membuka shift kasir</p><p className="mt-2">{sessionError}</p><Button className="mt-3" variant="outline" onClick={() => { setAutoOpenAttempted(false); void loadSession() }}>Coba lagi</Button></div>}{!sessionError && <div className="mt-6 flex items-center justify-center"><Loader2 className="size-6 animate-spin text-emerald-600" /></div>}</CardContent></Card></div>
  }
