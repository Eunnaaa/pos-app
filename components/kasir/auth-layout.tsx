...
export function AuthLayout({ children, title, description }: { children: React.ReactNode; title: string; description: string }) {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[1.05fr_1fr]" suppressHydrationWarning>
      <section className="relative hidden overflow-hidden bg-emerald-950 p-10 text-white lg:flex lg:flex-col" suppressHydrationWarning>
        ...
        <Link href="/" className="relative flex items-center gap-3" suppressHydrationWarning>
          <span className="flex size-10 items-center justify-center rounded-xl bg-emerald-500">
            <Store className="size-5" />
          </span>
          <span className="text-xl font-bold" suppressHydrationWarning>Kedai-Ku</span>
        </Link>
        <div className="relative my-auto max-w-xl">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-300">{t("smartPos")}</p>
          <h1 className="mt-5 text-4xl font-black leading-tight">{t("headline")}</h1>
          <p className="mt-5 text-lg leading-relaxed text-emerald-100/70">{t("subheadline")}</p>
      ...
      <section className="relative flex items-center justify-center bg-background px-4 py-10 sm:px-8" suppressHydrationWarning>
        <div className="absolute right-4 top-4"><LanguageToggle /></div>
        <div className="w-full max-w-md">
          <Link href="/" className="mb-10 flex items-center justify-center gap-2 lg:hidden" suppressHydrationWarning>
