...
export const GET = apiHandler(async (request) => {
  ...
  const [summary, trend, topProducts, lowStock, recentSales] = await Promise.all([
    ...
    `),
    db.execute(sql`
      select item_name as name, sum(quantity)::text as quantity, sum(total_amount)::text as sales
    ...
    db.execute(sql`
      ...
      where sb.organization_id = ${context.organizationId} and sb.available <= sb.reorder_point
